...
# Prefer double-clicking start-personal.bat in the project root.
...
if (-not (Test-Path $envFile)) {
    ...
    Write-Host 'Run: npm run setup-bridge -- -Token "YOUR_TOKEN"'
    exit 1
}
...
Write-Host "URL:  $url"
Write-Host ""
Write-Host 'Tip: double-click start-personal.bat to run the bridge in one window.' -ForegroundColor Cyan
Write-Host ""
Write-Host "Starting Codex bridge in a new window..." -ForegroundColor Green
$bridgeCmd = "cd `"$root`"; node scripts/start-personal.mjs"
...
Write-Host ""
Write-Host "[mcp_servers.developmentos]"
Write-Host 'command = "node"'
Write-Host "args = [""$mcpScript"", ""--token"", ""$token"", ""--url"", ""$url""]"
Write-Host ""
