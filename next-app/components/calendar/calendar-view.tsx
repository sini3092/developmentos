...
export function CalendarView({ workspaceId, year, month, events, projects }: CalendarViewProps) {
                  ...
                  {week.map((cell) => {
                    ...
                    return (
                      <button
                        key={cell.key}
                        type="button"
                        onClick={() => openCreateDialog(cell.key)}
                        className={cn(
                          "min-h-28 p-2 text-left transition-colors hover:bg-muted/30",
              ...
              <div className="mt-4 space-y-2">
                {events.length === 0 ? (
                  <p className="text-sm text-muted-foreground">
                    No deadlines or personal items this month. Click a day or use Add item.
                  </p>
                ) : (
                  events.slice(0, 12).map((event) =>
                    event.type === "personal" ? (
                      <button
                        ...
                        className={cn(
                          "block w-full rounded-lg border px-3 py-2 text-left text-sm transition-colors hover:border-info/40",
...
function CalendarEventChip({
  ...
  event: CalendarEvent
  onEditPersonal: (event: CalendarEvent) => void
}) {
