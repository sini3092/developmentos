...
import {
  ...
  archiveSoulsReportThreadsAction,
...
export function InboxChat({
  ...
  const soulsReportCount = useMemo(
    () => threads.filter((thread) => thread.kind === "souls_report").length,
    [threads]
  )

  function removeThread(threadId: string) {
    startTransition(async () => {
      await archiveInboxThreadAction(threadId)
          ...
          <div className="flex items-start justify-between gap-2">
            ...
            {soulsReportCount > 0 ? (
              <Button
                ...
                onClick={() =>
                  startTransition(async () => {
                    await archiveSoulsReportThreadsAction(workspaceId)
                    router.push("/inbox")
                    router.refresh()
                  })
                }
            ...
            <div className="space-y-1">
              {threads.map((thread) => (
                <div
                  key={thread.id}
                  className={cn(
                    "group flex items-start gap-2 rounded-xl px-3 py-2.5 transition-colors hover:bg-muted/50",
                    selectedThreadId === thread.id && "bg-muted",
                    thread.unread && "bg-violet-500/5"
                  )}
                >
                  <Link href={`/inbox?t=${thread.id}`} className="flex min-w-0 flex-1 items-start gap-2">
          ...
          <>
            <header className="flex items-center justify-between gap-3 border-b border-border/60 px-4 py-3">
              ...
              <Button
                ...
                variant="ghost"
                disabled={isPending}
                onClick={() => removeThread(selectedThread.id)}
              >
